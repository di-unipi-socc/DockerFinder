...
"""
This module interacts with the *Software service* running in the
*discovery* part.
"""
...
class ClientSoftware(requests.Session):
    ...
    def __init__(self, api_url="http://127.0.0.1:3001/api/software"):
        # ,host_service="sw_server", port_service="3001",
        # path_api="/api/software"):
    ...
    def get_software(self):
        ...
        A software is described by: the name, the version command, and
        theregex.\n
        ...
        try:
            ...
            if res.status_code == requests.codes.ok:
                ...
                self.logger.info(
                    str(json_response['count']) + " softwares received")
    ...
    def get_system(self):
        ...
        d = [('sh -c "cat /etc/*release"', '(?<=PRETTY_NAME=")[^"]*')]
        # ('sh -c "lsb_release -a"', 'Description:.*'))
    ...
    def post_software(self, dict_software):
        """Add a software."""
        try:
            res = self.post(self._url,
                            headers={'Content-type': 'application/json'},
                            json=dict_software)
            if res.status_code == requests.codes.created or\
               res.status_code == requests.codes.ok:
                self.logger.debug(
                    "POST [" + dict_software['name'] + "]  into  " + res.url)
            else:
