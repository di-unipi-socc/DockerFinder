...
docker service create  --name $SOFTWAREDB  \
...
docker service create   --name $SOFTWARE -p 3001:3001  \
  --constraint  $CONSTRAINT_NODE \
...
# Images database
docker service create  --name images_db  \
  --constraint  $CONSTRAINT_NODE \
...
docker service create  --name images_server  \
    --constraint $CONSTRAINT_NODE \
...
# RabbitMQ service
docker service create --name rabbitmq  -e HOSTNAME=rabbitmq \
  --constraint  $CONSTRAINT_NODE \
...
# Crawler service
docker service create   --name crawler \
  --constraint  $CONSTRAINT_NODE \
...
#Checker service
docker service create   --name checker  \
      --constraint  $CONSTRAINT_NODE \
...
###############   MONITOR PHASE ####################
#####################################################
docker service create  -p 3002:3002 --name monitor \
  --constraint  $CONSTRAINT_NODE \
